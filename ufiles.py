...
class Ufiles: #U-files, entorno gráfico e interacción de objetos
    ...
    def __init__(self): #Función constructora
        ...
        print lang(56),;self.root.iconbitmap(self.images.image("ucursos")) #icono del programa
        ...
        print lang(57),;self.root.bind("<Control-A>",_ayuda); self.root.bind("<Control-a>",_ayuda)
        ...
        self.root.protocol("WM_DELETE_WINDOW", self.salir); print lang(12); print lang(59)+"\n" #evento de salida
    ...
    def box_login(self): #Dibuja el box de login
        ...
        bt_iniciarSesion = Button(button1Frame,text=lang(10),font=tkFont.Font(family="Times",size=12),\
                                  relief=GROOVE, command=lambda:self.iniciar_sesion("box"),bg="#F6F7F8").pack(padx=0,pady=0)
    ...
    def cargar_curso(self,nombre,codigo,link,event): #Carga un curso
        def _delete_files_frame(): #Elimina el frame de los archivos
            ...
            self.background.delete('button4'); self.background.delete('button4:txt')
            self.background.delete('button5'); self.background.delete('button5:txt')
            self.background.delete('files'); self.background.delete('files_background')
            self.background.update()
        def _delete_loading_widget(): #Elimina el frame "cargando..."
            self.background.delete('loading_text'); self.background.delete('loading_widget'); self.background.delete('loading_background')
        def _files_frame(): #Dibuja el frame de los archivos
            ...
            self.background.create_text(PROGRAM_SIZE[0]-546,45, text=lang(41), font=self.fonts[7], fill="#666666", width=50, anchor=NW,tag="button1:txt")
            self.background.create_window(PROGRAM_SIZE[0]-463,26,window=button2,tag='button2')
            self.background.create_text(PROGRAM_SIZE[0]-487,45, text=lang(42), font=self.fonts[7], fill="#666666", width=50, anchor=NW,tag="button2:txt")
            self.background.create_window(PROGRAM_SIZE[0]-400,26,window=button3,tag='button3')
            self.background.create_text(PROGRAM_SIZE[0]-419,45, text=lang(66), font=self.fonts[7], fill="#666666", width=50, anchor=NW,tag="button3:txt")
            self.background.create_window(PROGRAM_SIZE[0]-335,26,window=button4,tag='button4')
            self.background.create_text(PROGRAM_SIZE[0]-364,45, text=lang(67), font=self.fonts[7], fill="#666666", width=60, anchor=NW,tag="button4:txt")
            self.background.create_window(PROGRAM_SIZE[0]-270,26,window=button5,tag='button5')
            self.background.create_text(PROGRAM_SIZE[0]-299,45, text=lang(68), font=self.fonts[7], fill="#666666", width=62, anchor=NW,tag="button5:txt")
        def _loading_frame(): #Dibuja el frame "cargando"
            ...
            self.background.create_text(PROGRAM_SIZE[0]/2-57,50,text=lang(63),font=self.fonts[5], anchor=W, tag='loading_widget')
        ...
        if self.loaded: #si se ha iniciado sesión
            ...
            if True: #Se carga el curso
                    ...
                    for s in range(1,3): #Se recorren los semestres
                        ...
                        for mat in U_CURSOS_MT:
                            for sec in range(1,U_CURSOS_SC+1): #Se recorren las secciones
                                consultas+=1
                                query_link = link+"/"+str(y)+"/"+str(s)+"/"+codigo+"/"+str(sec)+"/"+mat
                                if self.browser.abrirLink(query_link)==BR_ERRORxNO_ACCESS_WEB: #Si no se pudo cargar la web
                                    break
                                estado_carga = comprobarCarga(self.browser.getHtml())
                                if estado_carga == FILESXERROR_NOEXIST: break
                                elif estado_carga == FILESXERROR_NOPERM: pass
                                elif estado_carga == FILESXERROR_NOFILES: pass
                                elif estado_carga == FILESXNO_ERROR:
                                    self.files.append(buscarArchivos(self.browser.getHtml(), query_link,y,s,sec,mat))
                                time.sleep(0.5)
                _delete_loading_widget()
                _files_frame()
                self.activecurso = True; print lang(12)
                ...
                _delete_files_frame()
                self.root.title(PROGRAM_TITLE)
                self.box_error(CURSOSxERROR_CANT_LOAD_CURSO,codigo); print lang(61)
                self.activecurso = False; return
    ...
    def filtrar(self,filtro,event=None): #Se filtran y dibujan los resultados según un filtro
        ...
        if self.loaded: #Si se ha iniciado sesión
            if self.activecurso: #Si se ha cargado un curso
                ...
                if filtro=="material:docente": filtros = [0,4,1,5,6,7]; tipo = "material_docente"
                elif filtro=="material:alumnos": filtros = [0,1,4,5,6,7]; tipo = "material_alumnos"
                elif filtro=="fecha:todo": filtros = [8,6,7,0,4,1]
                elif filtro=="integrantes:todo": filtros = [4,0,1,2]
                elif filtro=="ver:filtros": return
                elif filtro=="ver:busqueda": filtros = [0,1,4,5,6,7,8,2,3]
                ...
                if filtro != "ver:busqueda":
                    ...
                    for elem in event:
                        data = []
                        for i in filtros: data.append(str(self.files[elem[0]][elem[1]][i]).decode('utf-8',errors='ignore'))
    ...
    def iniciar_sesion(self,event=None): #Iniciar sesión
        if event=="box": pass #si se inició sesión desde el box
            ...
            self.cerrar_sesion()
            return
        print lang(60),; login = [self.loginUsername.get(),self.loginPassword.get()]
        if self.browser.abrirLink(HREF_UPASAPORTE)==BR_ERRORxNO_ACCESS_WEB: #Si no se pudo cargar la web
            self.box_error(BR_ERRORxNO_ACCESS_WEB); print lang(61); return
        ...
        if estado is not True: self.box_error(estado); print lang(61); return #si no se inició la sesión
